...
static void _Check_XXX_BitBus(GUI_DEVICE * pDevice) {
  ...
  if ((pContext->HW_API.pfWrite16_A0  == NULL) ||
    ...
    GUI_Error(__FILE__": Missing hardware routines in function _Check_XXX_BitBus(). Execution stopped");
  }
  if ((pContext->UseCache == 0) && (pContext->HW_API.pfRead16_A1 == NULL)) {
    GUI_Error(__FILE__": Missing 16 bit hardware routines in function _Check_XXX_BitBus(). Execution stopped");
  }
}
...
static void _Init_XXX(GUI_DEVICE * pDevice) {
  ...
  pContext = (DRIVER_CONTEXT *)pDevice->u.pContext;
  if (pContext == NULL) {
    GUI_Error(__FILE__": Unknown error in function _Init_XXX(). Execution stopped");
  }
  ...
  if (pContext->BitsPerPixel != 8) {
    GUI_Error(__FILE__": Selected color depth currently not supported for this controller in function _Init_XXX(). Execution stopped");
  }
}
...
static int _Init(GUI_DEVICE * pDevice) {
  ...
  if (_InitOnce(pDevice)) {
    GUI_Error(__FILE__": Not enough memory available in function _Init(). Execution stopped");
  }
  if (pDevice->u.pContext) {
    pContext = (DRIVER_CONTEXT *)pDevice->u.pContext;
    //
    ...
    if (pContext->pfInit) {
      ...
      GUI_Error(__FILE__": No display controller selected in function _Init(). Execution stopped");
    }
  }
