...
*	返 回 值: 无
*********************************************************************************************************
*/	uint8_t font_width,font_height;uint8_t width;uint16_t font_bytes;
void LCD_DispStr(uint16_t _usX, uint16_t _usY, char *_ptr, FONT_T *_tFont)
{
	...
	uint8_t code2;
	uint32_t address = 0;
	uint8_t buf[96 * 96 / 8];	/* 最大支持24点阵汉字 */
	...
	else if(_tFont->FontCode == FC_ST_48)
	{
		font_height = 48;
		font_width = 48;
		font_bytes = 288;
	...
	else if(_tFont->FontCode == FC_ST_96)
	{
		font_height = 96;
		font_width = 96;
		font_bytes = 1152;
	...
	while (*_ptr != 0)
	{
		...
		for (m = 0; m < font_height; m++)	/* 字符高度 */
		{
			...
			{
				if ((buf[m * ((t * width) / font_width) + i / 8] & (0x80 >> (i % 8 ))) != 0x00)
				{
...
void LCD_DrawButton(BUTTON_T *_pBtn)
{
		...
		else if (_pBtn->Font->FontCode == FC_ST_48)
		{
			fwidth = 24;		/* 字体宽度 */
		}
		else
...
static void LCD_FSMCConfig(void)
{
	...
	timingWrite.FMC_AddressSetupTime = 4;
	timingWrite.FMC_AddressHoldTime = 0;
	timingWrite.FMC_DataSetupTime = 6;
	...
	*/
	...
	init.FMC_AsynchronousWait = FMC_AsynchronousWait_Disable;	/* 注意旧库无这个成员 */
