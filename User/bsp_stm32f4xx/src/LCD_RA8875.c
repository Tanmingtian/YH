...
*					(6) 解决 RA8875_DispBmpInFlash(), s_ucRA8875BusyNow 标志处理冲突的问题。
...
//static __IO uint8_t s_ucRA8875BusyNow = 0;			/* 忙标志, 其他模块通过 RA8875_IsBusy()函数判断忙 */
...
{
	s_ucRA8875BusyNow = 1;				/* 设置忙标志，避免触摸中断中的程序访问冲突 */
...
{
	s_ucRA8875BusyNow = 1;
	/* RA8875_WriteReg(0x50, 0x80);  不能使用这个函数，因为内部已经操作了 s_ucRA8875BusyNow 标志 */
