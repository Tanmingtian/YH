...
void bsp_InitExtSRAM(void)
{
	...
	*/
	...
	/*-- FSMC Configuration ------------------------------------------------------*/
	p.FMC_AddressSetupTime = 3;		/* 设置为2会出错; 3正常 */
	p.FMC_AddressHoldTime = 0;
	p.FMC_DataSetupTime = 2;			/* 设置为1出错，2正常 */
	p.FMC_BusTurnAroundDuration = 1;
	p.FMC_CLKDivision = 0;
	p.FMC_DataLatency = 0;
	p.FMC_AccessMode = FMC_AccessMode_A;

	FSMC_NORSRAMInitStructure.FMC_Bank = FMC_Bank1_NORSRAM3;
	FSMC_NORSRAMInitStructure.FMC_DataAddressMux = FMC_DataAddressMux_Disable;
	FSMC_NORSRAMInitStructure.FMC_MemoryType = FMC_MemoryType_SRAM;	// FSMC_MemoryType_PSRAM;
